...
model_name = "FacebookAI/roberta-base" # Changed to a BERT-like model for classification
#model_name = "microsoft/deberta-base" # Changed to a BERT-like model for classification
model_tag = model_name.split("/")[-1]+f"_peft_r{LORA_R}_a{LORA_ALPHA}"+str(lr).replace("-","_") # Extract model name for logging
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % checkpoint_save_epochs == 0:
        ...
        model.save_pretrained(checkpoint_dir) # Save the model in the PEFT format
