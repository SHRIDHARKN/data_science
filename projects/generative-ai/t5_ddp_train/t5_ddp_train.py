...
def train_and_evaluate(rank, world_size):
    ...
    log_dir = f"{GLOBAL_PROJ_FOLDER}/news_category/model/{GLOBAL_MODEL_TAG}/runs/{GLOBAL_MODEL_TAG}_classification_rank_{rank}"
    checkpoint_dir = f"{GLOBAL_PROJ_FOLDER}/news_category/model/{GLOBAL_MODEL_TAG}/checkpoints/{GLOBAL_MODEL_TAG}"
    ...
    # MODIFIED: Pass the tokenizer instance to the collate function via a lambda or functools.partial
    # This lambda is now *outside* train_and_evaluate, or defined very simply.
    # The tokenizer here is the one loaded by the main process (rank 0).
    # For num_workers > 0, the tokenizer in `t5_collate_fn_top_level` will be the one loaded by `worker_init_fn`.
    # This separation is key.
    
    # OPTION 1: Using functools.partial (cleaner than nested lambda)
    ...
    # OPTION 2: A simple lambda here, but it's okay because the tokenizer is a variable from this scope
    # and the collate_fn itself is top-level.
    # collate_fn_final = lambda batch: t5_collate_fn_top_level(
    #     batch, tokenizer_instance=tokenizer,
    #     model_max_length=GLOBAL_MODEL_CONTEXT_LENGTH,
    #     label_max_length=GLOBAL_LABEL_CONTEXT_LENGTH
    # )
    ...
    for epoch in range(GLOBAL_NUM_EPOCHS):
        ...
        if (epoch + 1) % checkpoint_save_epochs == 0 and rank == 0: # `checkpoint_save_epochs` is not global, assuming it's part of your constants
            ...
            torch.save({
                ...
                'model_name': GLOBAL_MODEL_NAME, # Use global constants for saving configs
                'lr': GLOBAL_LR,
                ...
                'model_context_length': GLOBAL_MODEL_CONTEXT_LENGTH,
                'label_context_length': GLOBAL_LABEL_CONTEXT_LENGTH,
            }, checkpoint_path)
    ...
    if rank == 0:
        ...
        # For evaluation on rank 0, you need its own tokenizer instance
        inference_tokenizer = AutoTokenizer.from_pretrained(GLOBAL_MODEL_NAME, cache_dir=GLOBAL_CACHE_DIR)
        
        # Create a collate_fn for the test dataloader using the inference_tokenizer
        ...
        inference_model = AutoModelForSeq2SeqLM.from_pretrained(GLOBAL_MODEL_NAME, cache_dir=GLOBAL_CACHE_DIR)

        checkpoint_files = [f for f in os.listdir(checkpoint_dir) if f.endswith('.pt')]
        if checkpoint_files:
            latest_checkpoint = max(checkpoint_files, key=lambda x: int(x.split('_')[2].split('.')[0]))
            checkpoint_path = os.path.join(checkpoint_dir, latest_checkpoint)
            print(f"Loading checkpoint for evaluation from {checkpoint_path}")
            checkpoint = torch.load(checkpoint_path, map_location=f"cuda:{rank}")
            inference_model.load_state_dict(checkpoint['model_state_dict'])
            inference_model.eval()
            print(f"Checkpoint loaded successfully for evaluation from {checkpoint_path}")
        else:
        ...
        for batch in loop:
            ...
            # Use the inference_tokenizer for processing inputs for generation
        ...
        results_dir = f'{GLOBAL_PROJ_FOLDER}/news_category/model/{GLOBAL_MODEL_TAG}/results'
